...
public class EditPersonnelSteps
{
    ...
    public void admin_enters_Address_Oak_Hills_Drive_City_Capitol_City_State_North_Carolina_Zip_Phone()
            throws Throwable
    {
        WebElement address1 = browser.findElement(By.cssSelector("input[name=\"streetAddress1\"]"));
        address1.sendKeys("123 Oak Hills Drive");

        WebElement city = browser.findElement(By.cssSelector("input[name=\"city\"]"));
        city.sendKeys("Capitol City");

        Select state = new Select(browser.findElement(By.cssSelector("select[name=\"state\"]")));
        state.selectByVisibleText("North Carolina");

        WebElement zip = browser.findElement(By.cssSelector("input[name=\"zip\"]"));
        zip.sendKeys("28700-0458");

        WebElement phone = browser.findElement(By.cssSelector("input[name=\"phone\"]"));
    ...
    public void admin_clicks_the_Edit_PHA_Record_button()
            throws Throwable
    {
        WebElement submit = browser.findElement(By.cssSelector("input[value=\"Edit Personnel Record\"]"));
        submit.click();
    }

    @Then("^A new Public Health Agent is added$")
    // with the following information: First Name: Tim Last Name: Agent Address: (\\d+) Oak Hills DriveCity: Capitol City State: North Carolina Zip:
    // (\\d+)-(\\d+) Phone: (\\d+)-(\\d+)-(\\d+) Email: pha@timagent\\.com$")
    public void a_new_Public_Health_Agent_is_added_with_the_following_information_First_Name_Tim_Last_Name_Agent_Address_Oak_Hills_DriveCity_Capitol_City_State_North_Carolina_Zip_Phone_Email_pha_timagent_com()
            throws Throwable
    {
        WebElement successMessage = browser.findElement(By.cssSelector("#iTrustContent span.iTrustMessage"));
